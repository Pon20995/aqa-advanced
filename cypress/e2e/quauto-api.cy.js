...
describe('API test for adding car', () => {
  ...
  const increasedMileageNumber = parseInt(mileageText, 10) + 100;
  const literText = 30;
  const costText = 45;
  ...
  it('Add expnses with command for ', () => {
    cy.makeExpenseRequestAndCheckResponse(
      carId,
      expectedDate,
      increasedMileageNumber,
      literText,
      costText,
      false,
    );
  ...
  it('Check expenses data on UI', () => {
    garagePage.fuelExpenseSidebarButton().click();
    fuelExpenses.fuelExpensesTitle().should('have.text', 'Fuel expenses');
    fuelExpenses.dateColuumn().should('have.text', expectedDateString);
    fuelExpenses.mileageColuumn().should('have.text', increasedMileageNumber);
